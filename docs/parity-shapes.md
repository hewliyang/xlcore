...
Shapes remain 🟡 — P1 #1–5 (gradient fills, direct `<a:effectLst>` outer shadow, style-ref matrix walk, group rotation + body-rect-follows-flip, preset dash + line cap/join on non-connector outlines) all shipped. Current v0 is good for basic callouts/buttons, themed gradients + drop shadows on direct *and* style-ref-driven DrawingML shapes, grouped/rotated screenshot chrome, and Office-authored org-chart / SOTP diagrams. Next obvious gaps are P1 #6 (`avLst` adjusts on `roundRect` / arrows / callouts — already extracted, ignored everywhere except braces) and P1 #7 (text autofit).
