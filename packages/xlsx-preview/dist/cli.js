...
// src/node.ts
import { Canvas, Image as Image2 } from "skia-canvas";
import { readFile } from "node:fs/promises";
import { readFileSync } from "node:fs";

// ../../crates/xlcore-wasm/pkg/xlcore_wasm.js
function extract_xlsx(bytes, options) {
  try {
...
function setOffscreenCanvasFactory(factory) {
...
// src/node.ts
setOffscreenCanvasFactory((width, height) => new Canvas(width, height));
if (typeof globalThis.Image === "undefined") {
  globalThis.Image = Image2;
}
var wasmReady = null;
async function loadWorkbookFromXlsx(input, options = {}) {
  await ensureWasm();
  const bytes = await bytesFromInput(input);
  return extract_xlsx(bytes, extractionOptions(options));
}
async function renderXlsxToPng(input, opts = {}) {
...
function renderToCanvas(layout, opts = {}) {
  ...
  const canvas = new Canvas(Math.ceil(viewport.w * (opts.zoom ?? 1) * (opts.scale ?? 1)), Math.ceil(viewport.h * (opts.zoom ?? 1) * (opts.scale ?? 1)));
  render(canvas, sheet, layout, {
    ...opts,
    viewport
  });
  return canvas;
}
async function renderToPng(layout, opts = {}) {
...
function loadOptionsFromRenderOptions(options) {
...
function pickSheet(layout, sheetIndex, sheetName) {
  ...
  if (!sheet)
    throw new Error(`sheetIndex out of range: ${index}`);
  return sheet;
}
function defaultViewport(sheet, opts) {
...
function viewportForRange(sheet, range) {
  const grid = buildGrid(sheet);
...
function parseArgs(argv) {
  const options = { scale: 1, info: false, all: false };
  for (let i = 0;i < argv.length; i++) {
    ...
    if (arg === "--input" || arg === "-i") {
      if (options.input)
        throw new Error("Input specified more than once");
      options.input = value;
      ...
      options.sheet = value;
    else if (arg === "--sheet-index")
      options.sheetIndex = Number(value);
    else if (arg === "--scale")
  ...
  if (!options.info && !options.output)
...
async function main() {
  ...
  if (options.all) {
    ...
    for (let i = 0;i < layout.sheets.length; i++) {
      const output2 = outputForSheet(resolve(options.output), layout.sheets[i]?.name ?? `Sheet${i + 1}`, i);
      const png2 = await renderToPng(layout, { range: options.range, sheetIndex: i, scale: options.scale });
      await mkdir(dirname(output2), { recursive: true });
      await writeFile(output2, png2);
      rendered.push({ sheetIndex: i, sheet: layout.sheets[i]?.name, output: output2, bytes: png2.length });
    }
...
function workbookInfo(input, layout) {
  return {
    input,
...
function outputForSheet(pattern, sheetName, index) {
