...
// src/react.ts
function useWorkbookPreviewer(file, options = {}) {
  ...
  return { containerRef, previewer, progress: progress2, error, loading };
}
function ExcelPreviewer(props) {
  const {
    file,
    previewerRef,
    className,
  ...
  useImperativeHandle(previewerRef, () => result.previewer, [result.previewer]);
...
function attachPreviewerEvents(previewer, optionsRef) {
  previewer.on("selectionchange", (event) => {
    optionsRef.current.onSelectionChange?.(event.detail);
  });
  previewer.on("sheetchange", (event) => {
    optionsRef.current.onSheetChange?.(event.detail);
  });
  previewer.on("zoomchange", (event) => {
    optionsRef.current.onZoomChange?.(event.detail);
  });
}
export {
  useWorkbookPreviewer,
  ExcelPreviewer
};
