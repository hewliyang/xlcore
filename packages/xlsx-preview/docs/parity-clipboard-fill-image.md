...
9. Delete drawing — Delete/Backspace on a selected chart/image fires `opts.onDrawingDelete` (`interact.ts`) → `previewer.ts` dispatches `drawingdeleted` (resolved via current anchor); `removeDrawing` worker op mirrors `moveDrawing` (`resolveDrawingId` → `charts.remove`/`images.remove`); example app patches the single-sheet layout.
...
10. External-paste type/formula handling — `pasteCells` worker op (`editWorker.ts`/`worker.ts`) applies each pasted cell like a manual edit: `=`-prefixed → `setFormula`, else `setValue(coerce(...))`. This handles numbers, booleans, and formulas in one per-cell pass (avoids the rectangular clobber where `set_range_formulas`/`set_range_values` each wipe the other's cells). Example app external-paste branch calls `recalcWorkbook.pasteCells({ row, column, values })`. Verified live: TSV with text + numbers + `=SUM(...)` → numbers right-aligned, formulas evaluate (405/295/215). TSV parser also drops a trailing-newline empty row. All gestures dogfooded live: external paste, internal copy/cut→paste, fill-handle drag, image drop, image clipboard paste, delete drawing.
11. Cut→paste move semantics — internal cut→paste now calls `moveRange` (xlcore-api `move_range` → wasm `moveRange` → `Range.moveTo` → `moveRange` worker op) instead of copy+clear. Move copies formulas verbatim (no relative shift), clears the source, then retargets every formula in the workbook whose reference falls inside the moved rect by the move delta — ignoring `$` (absolute refs follow moved cells too) and honoring sheet qualifiers. Copy→paste still shifts relative refs (`translate_formula_refs`); only cut differs. Rust tests in `tests/structural.rs` (`move_range_*`); verified live: cut `B1(=A1)`→D1 stays `=A1`; cut `A1`→A3 retargets external `=A1` formulas to `=A3`.
