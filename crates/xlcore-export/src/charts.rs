...
pub fn extract(
    ...
    let chart_parts: Vec<_> = drawings_part.chart_parts(doc).map(|p| p.clone()).collect();
    let image_parts: Vec<_> = drawings_part.image_parts(doc).map(|p| p.clone()).collect();
            ...
            xdr::WorksheetDrawingChoice::XdrTwoCellAnchor(a) => {
                ...
                let target = match a.two_cell_anchor_choice.as_ref()? {
                    xdr::TwoCellAnchorChoice::XdrGraphicFrame(gf) => {
                        ...
                        AnchorTarget::Chart(find_relationship_id(&gf.graphic.graphic_data.xml_children)?)
                    }
    ...
    let mut chart_by_rid: Vec<(String, ooxmlsdk::parts::chart_part::ChartPart)> = chart_parts
        ...
        .collect();
    let mut image_by_rid: Vec<(String, ooxmlsdk::parts::image_part::ImagePart)> = image_parts
        .into_iter()
        .filter_map(|p| Some((part_relationship_id_dbg(&p)?, p)))
        .collect();
    ...
    for (anchor, target) in anchors_xml {
        match target {
            AnchorTarget::Chart(rid) => {
                let pos = match chart_by_rid.iter().position(|(r, _)| r == &rid) { Some(i) => i, None => continue };
                let (_, cp) = chart_by_rid.remove(pos);
                let space = match cp.root_element(doc) { Ok(s) => s, Err(_) => continue };
                let chart = extract_chart(space, theme);
                out.push(Drawing { kind: "chart".to_string(), anchor, chart, image: None });
            }
            AnchorTarget::Image(rid) => {
                let pos = match image_by_rid.iter().position(|(r, _)| r == &rid) { Some(i) => i, None => continue };
                let ip = &image_by_rid[pos].1;
                use ooxmlsdk::sdk::SdkPart;
                let bytes = match ip.data(doc) { Some(b) => b.to_vec(), None => continue };
...
fn sniff_image_mime(b: &[u8]) -> Option<&'static str> {
    if b.len() >= 8 && &b[..8] == [0x89, b'P', b'N', b'G', 0x0D, 0x0A, 0x1A, 0x0A] { return Some("image/png"); }
    if b.len() >= 3 && b[0] == 0xFF && b[1] == 0xD8 && b[2] == 0xFF { return Some("image/jpeg"); }
    if b.len() >= 6 && (&b[..6] == b"GIF87a" || &b[..6] == b"GIF89a") { return Some("image/gif"); }
    if b.len() >= 2 && &b[..2] == b"BM" { return Some("image/bmp"); }
    if b.len() >= 12 && &b[..4] == b"RIFF" && &b[8..12] == b"WEBP" { return Some("image/webp"); }
    if b.len() >= 5 && (b.starts_with(b"<svg") || b.starts_with(b"<?xml")) { return Some("image/svg+xml"); }
    None
}
        ...
        ($coll:expr) => {{
            ...
            for ser in $coll {
                let mut row = common_series(
                    &ser.order, ser.series_text.as_deref(),
                    ser.chart_shape_properties.as_deref(),
                    ser.c_val.as_deref(), &ser.c_d_pt, theme,
                );
        ...
        match choice {
            c::PlotAreaChoice::CBarChart(bc) => {
                ...
                if let Some(g) = &bc.bar_grouping {
                    grouping = g.val.as_ref().map(|v| format!("{:?}", v).to_ascii_lowercase());
                }
            ...
            c::PlotAreaChoice::CLineChart(lc) => {
                ...
                grouping = lc.grouping.val.as_ref().map(|v| format!("{:?}", v).to_ascii_lowercase());
            ...
            c::PlotAreaChoice::CScatterChart(sc) => {
                ...
                scatter_style = sc.scatter_style.val.as_ref()
                    .map(|v| match v {
                ...
                for ser in &sc.c_ser {
                    let mut row = common_series_scatter(
                        &ser.order, ser.series_text.as_deref(),
                        ser.chart_shape_properties.as_deref(),
                        ser.c_y_val.as_deref(), &ser.c_d_pt, theme,
                    );
            ...
            c::PlotAreaChoice::CBar3DChart(_) => { chart_type = "bar".into(); break; }
            _ => {}
        }
    }
...
fn extract_data_labels(dl: Option<&c::DataLabels>) -> Option<DataLabels> {
    ...
    let show_value = seq.show_value.as_ref().map_or(false, |b| b.val.unwrap_or(true));
    let show_category = seq.show_category_name.as_ref().map_or(false, |b| b.val.unwrap_or(true));
    let show_series_name = seq.show_series_name.as_ref().map_or(false, |b| b.val.unwrap_or(true));
    let show_percent = seq.show_percent.as_ref().map_or(false, |b| b.val.unwrap_or(true));
    if !show_value && !show_category && !show_series_name && !show_percent {
        return None;
    }
    let position = seq.data_label_position.as_ref()
        .map(|p| match p.val {
    ...
    let num_fmt = seq.numbering_format.as_ref().map(|nf| nf.format_code.as_str().to_string());
    Some(DataLabels {
        show_value, show_category, show_series_name, show_percent,
        position, separator, num_fmt,
    })
}
...
fn extract_point_colors(
    ...
    if d_pts.is_empty() { return Vec::new(); }
    let max_idx = d_pts.iter().map(|dp| dp.index.val as usize).max().unwrap_or(0);
...
fn ax_data_values(cat: Option<&c::CategoryAxisData>) -> (Vec<String>, Option<String>) {
    let Some(cat) = cat else { return (Vec::new(), None); };
    let Some(choice) = cat.category_axis_data_choice.as_ref() else { return (Vec::new(), None); };
    match choice {
        c::CategoryAxisDataChoice::CStrRef(sr) => {
            (string_cache_values(&sr.string_cache), Some(sr.formula.as_str().to_string()))
        }
        c::CategoryAxisDataChoice::CNumRef(nr) => {
            let vals = nr.numbering_cache.as_ref()
                .map(|nc| nc.c_pt.iter().map(|p| p.numeric_value.as_str().to_string()).collect())
                .unwrap_or_default();
            (vals, Some(nr.formula.as_str().to_string()))
        }
        c::CategoryAxisDataChoice::CStrLit(lit) => (
            lit.c_pt.iter().map(|p| p.numeric_value.as_str().to_string()).collect(),
            None,
        ),
...
fn values_format(v: Option<&c::Values>) -> Option<String> {
    ...
    match v.values_choice.as_ref()? {
        c::ValuesChoice::CNumRef(nr) => nr.numbering_cache.as_ref()
...
fn y_values_values(v: Option<&c::YValues>) -> (Vec<f64>, Option<String>) {
    ...
    let Some(choice) = v.y_values_choice.as_ref() else { return (Vec::new(), None); };
    match choice {
        c::YValuesChoice::CNumRef(nr) => {
            let vals = nr.numbering_cache.as_ref().map(|nc| {
                let mut indexed: Vec<(u32, f64)> = nc.c_pt.iter()
        ...
        c::YValuesChoice::CNumLit(lit) => {
            let mut indexed: Vec<(u32, f64)> = lit.c_pt.iter()
                .filter_map(|p| p.numeric_value.as_str().parse::<f64>().ok().map(|v| (p.index, v)))
                .collect();
            indexed.sort_by_key(|(i, _)| *i);
...
fn y_values_format(v: Option<&c::YValues>) -> Option<String> {
    ...
    match v.y_values_choice.as_ref()? {
        c::YValuesChoice::CNumRef(nr) => nr.numbering_cache.as_ref()
            .and_then(|nc| nc.format_code.as_ref().map(|s| s.as_str().to_string())),
...
fn x_axis_values(x: Option<&c::XValues>) -> (Vec<String>, Option<String>) {
    let Some(x) = x else { return (Vec::new(), None); };
    let Some(choice) = x.x_values_choice.as_ref() else { return (Vec::new(), None); };
    match choice {
        c::XValuesChoice::CStrRef(sr) => (string_cache_values(&sr.string_cache), Some(sr.formula.as_str().to_string())),
...
fn series_text_or_ref(t: Option<&c::SeriesText>) -> (Option<String>, Option<String>) {
    ...
    let Some(choice) = t.series_text_choice.as_ref() else { return (None, None); };
    match choice {
        c::SeriesTextChoice::CStrRef(sr) => {
            let cached = sr.string_cache.as_ref()
                .and_then(|sc| sc.c_pt.first().map(|p| p.numeric_value.as_str().to_string()));
...
fn number_reference_values(v: Option<&c::Values>) -> (Vec<f64>, Option<String>) {
    let Some(v) = v else { return (Vec::new(), None); };
    let Some(choice) = v.values_choice.as_ref() else { return (Vec::new(), None); };
    match choice {
...
fn string_cache_values(sc: &Option<Box<c::StringCache>>) -> Vec<String> {
    ...
    let mut indexed: Vec<(u32, String)> = sc.c_pt.iter()
...
fn series_color_via_debug(props: Option<&c::ChartShapeProperties>, theme: Option<&Theme>) -> Option<String> {
    let props = props?;
    ...
    let end = fill_block.find("})),").or_else(|| fill_block.find("}))")).unwrap_or(fill_block.len());
    ...
    if let Some(p) = fill_block.find("RgbColorModelHex {") {
        ...
        if let Some(v) = rest.find("val: \"") {
            ...
            if let Some(e) = rest.find('"') {
                let hex = &rest[..e];
                if hex.len() == 6 { return Some(format!("#{}", hex)); }
            }
        }
    }
...
fn extract_title(t: Option<&c::Title>) -> Option<String> {
    ...
    match txt.chart_text_choice.as_ref()? {
        c::ChartTextChoice::CStrRef(sr) => sr.string_cache.as_ref()
            .and_then(|sc| sc.c_pt.first().map(|p| p.numeric_value.as_str().to_string())),
        ...
        c::ChartTextChoice::CStrLit(lit) => lit.c_pt.first()
