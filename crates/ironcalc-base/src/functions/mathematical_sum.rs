...
impl<'a> Model<'a> {
    // SUMPRODUCT(array1, [array2], ...) - Returns the sum of products of
    // corresponding entries. Non-numeric values are treated as zero.
    pub(crate) fn fn_sumproduct(&mut self, args: &[Node], cell: CellReferenceIndex) -> CalcResult {
        ...
        for arg in args {
            let (rows, cols, values) = match self.sumproduct_values(arg, cell) {
                Ok(values) => values,
                Err(error) => return error,
            };

            if let Some((expected_rows, expected_cols)) = shape {
                if rows != expected_rows || cols != expected_cols {
                    return CalcResult::new_error(
                        Error::VALUE,
                        cell,
                        "SUMPRODUCT arguments must have the same dimensions".to_string(),
                    );
                }
            } else {
                shape = Some((rows, cols));
                products.resize(values.len(), 1.0);
            }

            for (product, value) in products.iter_mut().zip(values.into_iter()) {
                *product *= value.unwrap_or(0.0);
            }
        }

        CalcResult::Number(products.into_iter().sum())
    }
    ...
    fn sumproduct_values(
        ...
        match self.evaluate_node_in_context(arg, cell) {
            ...
            CalcResult::Boolean(value) => Ok((1, 1, vec![Some(if value { 1.0 } else { 0.0 })])),
            CalcResult::EmptyCell | CalcResult::EmptyArg | CalcResult::String(_) => {
                Ok((1, 1, vec![None]))
            }
            CalcResult::Range { left, right } => {
                ...
                Ok((rows, cols, self.values_from_range(left, right)?))
            }
                ...
                let values = match self.values_from_array(array) {
                    ...
                    Err(error) => {
                        return Err(CalcResult::new_error(
                            error,
                            cell,
                            "Error in SUMPRODUCT array".to_string(),
                        ));
                    }
                };
                Ok((rows, cols, values))
